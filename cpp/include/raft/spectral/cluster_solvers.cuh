...
struct kmeans_solver_t {
                                              ...
                                              index_type_t* __restrict__ codes) const
  {
    ...
    rmm::device_uvector<value_type_t> centroids(config_.n_clusters * dim, handle.get_stream());

    raft::cluster::kmeans_fit_predict<value_type_t, index_type_t>(
      handle, km_params, obs, nullptr, centroids.data(), n_obs_vecs, dim, codes, residual, iters);
