  ...
  bool bv_is_empty                                                                   = true)
{
  ...
  raft::linalg::binary_div_skip_zero(
    handle, V, raft::make_const_mdspan(V_max), raft::linalg::Apply::ALONG_ROWS);

  if (!bv_is_empty) {
    raft::linalg::binary_div_skip_zero(
      handle, BV, raft::make_const_mdspan(V_max), raft::linalg::Apply::ALONG_ROWS);
  } else {
    if (B_opt)
  ...
  int verbosityLevel    = 0)
{
  ...
  auto eigVectorBuffer =
    rmm::device_uvector<value_t>(size_x * size_x, stream);  // rmm because of resize
  ...
  while (iteration_number < max_iter + 1) {
    ...
    if (iteration_number > 0) {
      ...
      activePView = raft::make_device_matrix_view<value_t, index_t, col_major>(
        activePbuffer.data(), n, currentBlockSize);
      activeAPView = raft::make_device_matrix_view<value_t, index_t, col_major>(
      ...
      if (B_opt.has_value()) {
        activeBPView = raft::make_device_matrix_view<value_t, index_t, col_major>(
    ...
    if (explicitGramFlag) {
      ...
      raft::linalg::gemm(handle, XTRowView, BX.view(), gramXBX.view());
      raft::linalg::gemm(
    ...
    if (verbosityLevel > 10) {
      ...
      raft::matrix::print(handle,
                          raft::make_device_matrix_view<const value_t, index_t, col_major>(
                            eigLambdaTempView.data_handle(), 1, eigLambdaTempView.extent(0)),
                          ps);
    }
    ...
    auto eigBlockVectorP = raft::make_device_matrix<value_t, index_t, raft::col_major>(
      handle, gramDim - (size_x + currentBlockSize), size_x);
    ...
    if (B_opt.has_value()) {
      ...
      raft::matrix::slice(handle,
                          make_const_mdspan(eigVectorView),
  ...
  raft::linalg::binary_mult_skip_zero(
    handle, aux.view(), make_const_mdspan(eigLambda.view()), raft::linalg::Apply::ALONG_ROWS);

  raft::linalg::subtract(
    handle, raft::make_const_mdspan(AX.view()), raft::make_const_mdspan(aux.view()), R.view());

  raft::linalg::reduce(residual_norms.data_handle(),
                       R.data_handle(),
                       size_x,
                       n,
                       value_t(0),
                       ...
                       false,
                       raft::sq_op());
  // TODO check reduce sqrt postop raft::linalg::sqrt(handle,
  // raft::make_const_mdspan(aux_sum.view()), residual_norms.view());
