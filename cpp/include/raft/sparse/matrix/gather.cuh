...
template <typename MatrixIteratorT,
          ...
          typename IndexT = int>
__global__ void gatherKernelCsr(const MatrixIteratorT in_val,
                                const IndicesIteratorT in_ind,
                                const IndicesIteratorT in_indptr,
                                IndexT nnz,
                                IndexT D, // n_cols
                                IndexT N, // n_rows
                                IndexT m, // maplength
                             ...
                             MapTransformOp transform_op)
{
  ...
  if (predicate) {
    IndexT inRow = transform_op(map_val);
    IndexT inRowStartIdx = in_ind[inRow];
    IndexT inRowStopIdx  = get_stop_idx(inRow, m, nnz, in_ind);

    for (int j = start_idx; j < stop_idx; j++) {
      out[outRowStart + in_indptr[j]] = in_vals[inRowStartIdx + j];
    }
  }
}
